...
dummies_apacheadmissiondx <- dummy_cols(final_df_er_sepsis$apacheadmissiondx, remove_first_dummy = FALSE)
...
final_df_er_sepsis <- data.frame(final_df_er_sepsis,dummies_unittype[,2:8],dummies_apacheadmissiondx[,2:7])
```
#Creating functions
```{r}
# Metric performance
...
"Intercept", "Sex (female)", "Age (>16- 90)", "Ethnicity: Asian", "Ethnicity: Caucasian", "Ethnicity: Hispanic", "Ethnicity: American", "Ethnicity: Other", "ID hospital", "Time in emergency before ICU", "Sepsis", "Infection", "apache score IV", "bact not resistant", "bact resistant", "Charlson score", "History metastasis", "History vih/sida", "History cirrosis", "History stroke", "History renal", "History diabetes", "History cancer", "History leukemia", "History lymphoma", "History myocardial infar", "History chf", "History pvd", "History tia", "History dementia", "History copd", "History ctd", "History pud", "History liver", "Unit Type CCU CTICU", "Unit type CSICU", "Unit type CTICU", "Unit type Med.surg.ICU", "Unit type MICU", "Unit type Neuro ICU", "Unit type SICU", "Apache admission: sepsis,GI", "Apache admission:sepsis,cutaneous/soft tissue", "Apache admission:sepsis,pulmonary", "Apache admission:sepsis,renal/UTI", "Apache admission: sepsis unknown", "Apache admission:other", "er_los_hrs = 0.4068635", "er_los_hrs = 2.448543", "er_los_hrs = 1.1447e-17"
 ))
  ...
  #filter(predictor != "(Intercept)") %>%
   mutate(
    ...
    predictor = case_when(
      ...
      predictor == "sepsis_bin1" ~"Sepsis",
      predictor == "infection_bin1" ~ "Infection",
      ...
      predictor == "hist_vih_sida_bin" ~ "History vih/sida",
      ...
      predictor == ".data_Sepsis..GI" ~ "Apache admission: sepsis,GI",
      predictor == ".data_Sepsis..cutaneous.soft.tissue" ~ "Apache admission:sepsis,cutaneous/soft tissue",
      predictor == ".data_Sepsis..pulmonary" ~ "Apache admission:sepsis,pulmonary",
      predictor == ".data_Sepsis..renal.UTI..including.bladder." ~ "Apache admission:sepsis,renal/UTI",
      predictor == ".data_Sepsis..unknown" ~ "Apache admission: sepsis unknown",
      predictor == ".data_Other" ~ "Apache admission:other",
      ...
      predictor == "s(er_los_hrs).3" ~ "er_los_hrs = 1.1447e-17"#a tercera parte del rango de er_los_hrs, un aumento de una unidad en er_los_hrs está asociado con un cambio muy pequeño (casi cero) en el log-odds de unitdischargestatus
    ),
