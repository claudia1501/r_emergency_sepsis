...
pred <- marginaleffects::predictions(gam_vaso, newdata = final_df_er_sepsis, type = "response")

# Convertir a un dataframe
pred_df <- as.data.frame(pred)

# Multiplicar los valores predichos por 100
pred_df$estimate <- pred_df$estimate * 100
pred_df$conf.low <- pred_df$conf.low * 100
pred_df$conf.high <- pred_df$conf.high * 100

# Hacer el plot con los valores ajustados
plot2 <- ggplot(pred_df, aes(x = er_los_hrs, y = predicted, col = factor(vasopressors), fill = factor(vasopressors))) + 
...
variables_order <- rev(c(
  "Intercept", "ICU admission time (hours) and administration of vasopressors", "Sex (Male)", "Age (years)", "Ethnicity:", "ApachescoreIV", "Final Charlson Score" # "Renal History", "Cancer History", "CHF History", "COPD History", "Other History"
))
...
or_icudeath_gam2 <- or_icudeath_gam %>%
  mutate(
    term = case_when(
      ...
      term == "er_los_hrs:vasopressors" ~ "ICU admission time (hours) and administration of vasopressors",
      term == "sex" ~ "Sex (Male)",
      term == "age" ~ "Age (years)",
...
ggsave("C:/Users/cjimenez10J/Documents/projects/r_urgencias_sepsis/r_urgencias_sepsis/Figures/forestplot_gam_vaso.png", plot = forest_plot, width = 8, height = 6)
```
