...
diabetes_completo <- run_query(getSQL("C:/Users/cjimenez10J/Documents/projects/r_urgencias_sepsis/r_urgencias_sepsis/Analysis/sql/diabetes_completo.sql"))
diabetes_doble <- run_query(getSQL("C:/Users/cjimenez10J/Documents/projects/r_urgencias_sepsis/r_urgencias_sepsis/Analysis/sql/diabetes_doble.sql"))
diabetes_uno <- run_query(getSQL("C:/Users/cjimenez10J/Documents/projects/r_urgencias_sepsis/r_urgencias_sepsis/Analysis/sql/diabetes_uno.sql"))
diabetes_dos <-  run_query(getSQL("C:/Users/cjimenez10J/Documents/projects/r_urgencias_sepsis/r_urgencias_sepsis/Analysis/sql/diabetes_dos.sql"))
```
...
```{r}
final_df<- final_df %>%
  mutate(hist_diabetes = replace_na(hist_diabetes, 0)) %>%
  mutate(apache_diabetes = replace_na(apache_diabetes, 0)) %>%
  mutate(diagnosis_diabetes = replace_na(diagnosis_diabetes, 0))
 

  
final_df <- final_df %>%
  mutate(diabetes = ifelse(hist_diabetes == 1 | apache_diabetes == 1 | diagnosis_diabetes == 1, 1, 0))
...
final_df_er_sepsis <- final_df %>%
  select(unitdischargestatus,er_los_hrs, sex, age, ethnicity, apachescoreIV, final_charlson_score,apacheadmissiondx, vasopressors) #hist_renal_bin, hist_diabetes_bin, hist_cancer_bin, hist_chf_bin, hist_copd_bin,hist_other_bin
