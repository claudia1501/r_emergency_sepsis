...
# Vuelve a contar la frecuencia después de la agrupación
new_unittype_counts <- final_df %>%
  count(unittype, name = "Frequency")
# Muestra el resultado
print(new_unittype_counts)
...
# Incluir en df 
final_df_order <- data.frame(final_df_order,dummies_unittype[,2:6])
...
final_df_order <- data.frame(final_df_order,dummies_apacheadmissiondx[,2:10])
...
final_df_er_sepsis <- final_df_order %>%
  select(patientunitstayid, sex,age,ethnicity,er_los_hrs,unitdischargestatus,apachescoreIV,bact_resist_level, final_charlson_score, hist_stroke_bin,hist_renal_bin,hist_diabetes_bin,hist_cancer_bin,hist_myocardial_infar_bin,hist_chf_bin,hist_copd_bin,unit_med_surg_icu,unit_micu,apacheadmissiondx_other, apacheadmissiondx_pneumonia_bacterial, apacheadmissiondx_sepsis_pulmonary, apacheadmissiondx_sepsis_renal_uti, apacheadmissiondx_sepsis_unknown) #sepsis_bin,infection_bin
```
