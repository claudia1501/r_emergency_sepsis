...
duplicados <- final_df[duplicated(final_df$patientunitstayid) | duplicated(final_df$patientunitstayid, fromLast = TRUE), ]
print(duplicados)
final_df_sin_duplicados <- final_df[!duplicated(final_df$patientunitstayid), ]

# Verificar que no hay duplicados
contados <- table(final_df_sin_duplicados$patientunitstayid)
print(contados[contados > 1])
```
...
final_df_er_sepsis <- final_df %>%
  select(patientunitstayid,unitdischargestatus,er_los_hrs, sex, age, ethnicity, apachescoreIV, final_charlson_score, vasopressors) #hist_renal_bin, hist_diabetes_bin, hist_cancer_bin, hist_chf_bin, hist_copd_bin,hist_other_bin
