...
final_df <- final_df %>%
  mutate(
    apacheadmissiondx_recoded = recode(
      apacheadmissiondx,
      !!!setNames(rep("Other", length(unique(final_df$apacheadmissiondx))), setdiff(unique(final_df$apacheadmissiondx), top_categories))
    )
  )

# Opcional: Eliminar la variable original y renombrar la nueva variable recodificada
final_df <- final_df %>%
  select(-apacheadmissiondx) %>%
  rename(apacheadmissiondx = apacheadmissiondx_recoded)
